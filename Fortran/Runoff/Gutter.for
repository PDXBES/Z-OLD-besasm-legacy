C     Last change:  AMM   4 Jan 2002    1:02 pm
...
C=======================================================================
cc      write(*,*)' in gutter  MTIME NG NOG ',MTIME,NG,NOG
cc      write(*,*)' nscrat(2) ',NSCRAT(2)
cc           pause
...
C=======================================================================
     ...
     +                            NN=1,NQS)
           WRITE(N6,57)     IDASH,(IDASH,NN=1,NQAL+1)
           RETURN
           ENDIF
C#### WCH, 4/7/94.  INITIALIZE KWIKGT
cc      write(*,*)' in gutter line 122 NOG NSAVE ',NOG,NSAVE
      KWIKGT = 0
      NOGG                = NOG + NSAVE
...
C#######################################################################
cc      write(*,*)' line 240 J QIN(J) ',J,QIN(J)
cc      write(*,*)' line 240 nscrat(2) ',NSCRAT(2)
      IF(QINTOT(J) .GT. MAXQIN(J)) MAXQIN(J) = QINTOT(J)
cc      write(*,*)' after qintot nscrat(2) ',NSCRAT(2)
 
      IF(NPG(J).EQ.3) THEN 
cc           write(*,*)' after npg(j) nscrat(2) ',NSCRAT(2)
           ...
           ENDIF
  289 CONTINUE
cc      write(*,*)' before statement 290 '
cc      write(*,*)' after statement 289 nscrat2',NSCRAT2
cc      write(*,*)' nscrt2 ',NSCRT2
c
cc      pause
  290 CONTINUE
C=======================================================================
...
C=======================================================================
  ...
  305           CONTINUE
           ...
           STGA(LU)   = STG(LB)
 291       CONTINUE
           WRITE(NSCRT5,ERR=776) JULDAY,TIMDAY,DELT,
...
cc           write(*,*)' N FLOWED(N) ',N,FLOWED(N)
  500      CONTINUE
cc           write(*,*)' after statement 500 nscrat2',NSCRAT2
cc           write(*,*)' nscrat(2) ',NSCRAT(2)
c
           if(NSCRT2 .EQ. 0 )NSCRT2 = NSCRAT(2)
 
           WRITE(NSCRT2,ERR=777) JULDAY,TIMDAY,DELT,
     +                           (FLOWED(N),N=1,NPRNT)
cc           write(*,*)' after writing scratch 2 '
           IF(NQS.NE.0) THEN
               DO 520 N    = 1,NPRNT
               J           = IABS(IPRNT(N))
               DO 520 K    = 1,NQS
...
C=======================================================================
 ...
 779  WRITE(N6,9779)
      STOP
C=======================================================================
C AMM 1/4/2002 Increased year size for Y2K
 40   FORMAT(/,1H1,/,
     +' ***********************************************',/,
...
C     +' * ',10X,A10,' 19',I2,10X,'         *',/,
     +' * ',10X,A10,' ',I4,10X,'         *',/,
     +' ***********************************************',//,
     1  ' Day',T7,'Inlet',T13,'Rain',T22,'Flow',T30,10(1X,A8,1X))
C AMM 1/4/2002 Increased year size for Y2K
 50   FORMAT(/,1H1,/,
     +' ***********************************************',/,
     +' * Summary of Quantity and Quality results for *',/,
C     +' * ',16X,' 19',I2,14X,'         *',/,
     +' * ',16X,' ',I4,14X,'         *',/,
     +' ***********************************************',//,
     1 ' Month',T7,' Inlt',T13,'Rain',T22,'Flow',T30,10(1X,A8,1X))
 55   FORMAT(              T13,'Inch',T22,'Inch',T30,10(A2,A8))
 56   FORMAT(              T13,' mm ',T22,' mm ',T30,10(A2,A8))
 57   FORMAT(2X,A4,' ---',T13,A4,T20,A8,T30,10(1X,A8,'-'))
 340  FORMAT(/,
 ...
 343  FORMAT(/,
     ...
     2' ======> Time   =',1PE9.2,'  hours.  Occurrence #',I9,/,
     3' ======> Volume =',1PE9.2,' cub met. Full flow =',0PF9.1,' cms.')
  415 FORMAT(1X,I3,T6,I5,1X,F8.5,1X,F7.3,T30,10(G10.3))
  416 FORMAT(1X,I3,T6,A5,1X,F8.5,1X,F7.3,T30,10(G10.3))
  460 FORMAT(1X,A10,T6,I5,1X,F8.5,1X,F7.3,T30,10(G10.3))
  461 FORMAT(1X,A10,T6,A5,1X,F8.5,1X,F7.3,T30,10(G10.3))
  470 FORMAT('Total ',I4,1X,F8.5,1X,F7.3,T30,10(G10.3))
  471 FORMAT('Total ',A4,1X,F8.5,1X,F7.3,T30,10(G10.3))
  510 FORMAT(' Year ',I4,1X,F8.5,1X,F7.3,T30,10(G10.3))
  511 FORMAT(' Year ',A4,1X,F8.5,1X,F7.3,T30,10(G10.3))
  815 FORMAT(1X,I3,T6,I5,1X,F8.2,1X,F7.3,T30,10(G10.3))
  816 FORMAT(1X,I3,T6,A5,1X,F8.2,1X,F7.3,T30,10(G10.3))
  860 FORMAT(1X,A10,T6,I5,1X,F8.2,1X,F7.3,T30,10(G10.3))
  861 FORMAT(1X,A10,T6,A5,1X,F8.2,1X,F7.3,T30,10(G10.3))
  870 FORMAT('Total ',I4,1X,F8.2,1X,F7.3,T30,10(G10.3))
  871 FORMAT('Total ',A4,1X,F8.2,1X,F7.3,T30,10(G10.3))
  910 FORMAT(' Year ',I4,1X,F8.2,1X,F7.3,T30,10(G10.3))
  911 FORMAT(' Year ',A4,1X,F8.2,1X,F7.3,T30,10(G10.3))
