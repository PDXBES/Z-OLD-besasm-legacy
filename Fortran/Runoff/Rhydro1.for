C     Last change:  AMM   6 Jun 2001    3:16 pm
      SUBROUTINE RHYDRO1
C=======================================================================
...
C=======================================================================
      ...
      CHARACTER GOUT(20)*60,FOR1(2)*60,FOR2(2)*60,FOR3(2)*60,FOR4(2)*60
C#### WCH, 12/92
     ...
     1           '  Dummy  ','Parabolic'/
      ...
      GOUT(1) = '('' Input   NAMEG:  Drains                         '','
      GOUT(2) = '(''Sequen  Channel   to     Channel  Width  Length '','
      GOUT(3) = '(''Number    ID #   NGTO:    Type     (ft)   (ft)  '','
      GOUT(4) = '(''------  --------------- -------- ------ ------- '','
     ...
     +s)'')'
      GOUT(8) = '''  ------- -------- ------- ------- ------- -----  ---
     +--'')'
      GOUT(9) = '(''Number    ID #   NGTO:    Type     (m)    (m)   '','
...
C=======================================================================
     ...
     1                   IVAP,NHR,NMN,NDAY,MONTH,NYEAR
...
C=======================================================================
      IPRNGW = 0
      READ(N5,*,ERR=88) CC,IPRN(1),IPRN(2),IPRN(3),IPRNGW
      GO TO 90
C#######################################################################
C WCH, 12/92.  TRY ADDING OPTIONAL PRINT CONTROL FOR GROUNDWATER ERRORS.
C ASSUME IF THERE IS A READ ERROR, IT IS DUE TO EXTRA IPRNGW
C#######################################################################
   88      BACKSPACE N5
           BACKSPACE N5
           READ(N5,*,ERR=888) CC,IPRN(1),IPRN(2),IPRN(3)
   90 WRITE(N6,40)          IPRN(1),IPRN(2),IPRN(3)
...
C#### WCH, 12/5/94.  CHECK FOR PRESENCE OF F1 LINE.
                    ...
                    IF(NVAP(1).GT.100) CALL ERROR('152')
...
C#### WCH, 10/5/93.
  ...
  625 FORMAT(/,1H1,/
     ...
     +' **********************************************************',/)
  630 FORMAT(1X,I3,2X,I8,I8,1X,A9,F6.1,F8.1,F10.4,
     1                             F9.4,F8.4,2F8.1,F8.4,1PE9.2)
  631 FORMAT(1X,I3,1X,A8,1X,A8,1X,A9,F6.1,F8.1,F10.4,
     1                             F9.4,F8.4,2F8.1,F8.4,1PE9.2)
  635 FORMAT(1X,I3,2X,I8,I8,1X,A12,F7.1,F12.3,5X,F12.4)
  636 FORMAT(1X,I3,1X,A8,1X,A8,1X,A12,F7.1,F12.3,5X,F12.4)
  950 FORMAT(/,1H1,/
     +' *********************************************************',/,
 ...
 1095 FORMAT(    16X,'Tributary Subareas........',5I10)
 1096 FORMAT(    16X,'Tributary Subareas........',5(1X,A10))
 1120 FORMAT(//,2X,'   INLET')
 1170 FORMAT(1X,I10,5X,'Tributary Channel/Pipes...',5I10)
