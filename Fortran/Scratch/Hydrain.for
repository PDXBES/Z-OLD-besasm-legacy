C     Last change:  AMM   8 Jul 2002    5:23 pm
      SUBROUTINE HYDRAIN
C======================================================================
...
*
                 JSTA = 1
* AMM 1/30/2002 Removed Y2K Dependency
*                 IYR = NYR - 1900
                 IYR = NYR
...
C
         ...
         IF(N7.GT.0) THEN
           IF (NTYPE.GT.1) THEN
             WRITE(N7,ERR=20)NSTA,MRAIN,JSTA
           ELSE
             WRITE(N7,ERR=20)NSTA,MRAIN,'       1'
           END IF
         ENDIF
         JDAY=ISDATE
           DO 10 I=1,1000000
...
*
*
             IF(NTYPE.GT.1) HOUR = IHR*3600 + INC*ITIM*60
*
...
210   FORMAT(7X,'Please Select Input File Format:',/,12X,
     ...
     + 'COLUMN FIXED FORMAT FILE ............ 2',/,12X,
     + 'Your Choice -> ',\)
*
*
...
500   FORMAT(3X,'Please Enter the name of the Rainfall Interface File',
     + ' to be created -> '\)
600   FORMAT(I4,I2,I3,2I2,I3)
700   FORMAT(I10,F8.0,F8.3)
800   FORMAT(I4,'/',I2,'/',I2,F8.0,F8.3)
2000  FORMAT( /,' >>>>  READING AND CONVERTING HYDRA RAIN FILE '//)
2100  FORMAT(/' STARTING DATE ON RAIN FILE IS: ',I2,'/',I2,'/',I4/
