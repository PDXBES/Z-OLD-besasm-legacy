C     Last change:  AMM   7 Aug 2000   11:38 am
C#######################################################################
C            EPA Storm Water Management Model Main Program
C
C     NOTE THIS VERSION IS SPECFIC TO PDX VERSION OF SWMM 4.20
C
...
C=======================================================================
      ...
      N55   = 55
cpdx--plj, 11/1/91------------------------------------------------------
cpdx         unit numbers were never assigned for input and output files
cpdx         lack of identification caused trouble in the combine block
        N5 = 25                 !pdx
        N6 = 26                 !pdx
cpdx--------------------------------------------------------------------
      JCE   = 0
...
C=======================================================================
 3200 CALL STRIP(N55)
      ...
      WRITE(N6,15)
      DO 80 I = 1,NBLOCK
      IF(JIN(I).EQ.5.OR.JIN(I).EQ.6.OR.JIN(I).EQ.55) CALL ERRORSW(117)
...
cpdx                          ENDIF
      IF(CNAME.EQ.'COMB') THEN
                          ...
                          IFIND = 1
                          ENDIF
cpdx      IF(CNAME.EQ.'RAIN') THEN
cpdx                          CALL RAIN
cpdx                          IFIND = 1
cpdx                          ENDIF
cpdx      IF(CNAME.EQ.'TEMP') THEN
cpdx                          CALL TEMP 
cpdx                          IFIND = 1
cpdx                          ENDIF
cpdx      IF(CNAME.EQ.'RUNO') THEN
cpdx                          CALL RUNOFF
cpdx                          IFIND = 1
cpdx                          ENDIF
cpdx      IF(CNAME.EQ.'TRAN') THEN
cpdx                          CALL TRANS
cpdx                          IFIND = 1
cpdx                          ENDIF
cpdx      IF(CNAME.EQ.'STOR') THEN
cpdx                          CALL STRT
cpdx                          IFIND = 1
cpdx                          ENDIF
cpdx      IF(CNAME.EQ.'STAT') THEN
cpdx                          CALL STATS
cpdx                          IFIND = 1
cpdx                          ENDIF
cpdx      IF(CNAME.EQ.'EXTR') THEN
cpdx                          CALL EXTRAN
cpdx                          IFIND = 1
cpdx                          ENDIF
...
11    FORMAT(//T11,
     4' -------------------------------------------------',/,T11,
     +' |                                               |',/,T11,
     +' |  UPDATED FOR THE CITY OF PORTLAND, ORE. BES   |',/,T11,
     +' |                                               |',/,T11,
     +' |        by Brown & Caldwell and CH2M  1991     |',/,T11,
...
*     3'*************************************************',/)
  ...
  300 FORMAT(/,
     ...
     +'  Runoff Water quality constituents (NRQ).............',I5,/,
     +'  Runoff Land Uses per Subcatchment (NLU).............',I5,/,
