...
var
	DataList,CaptionList,VisibleList,HandleList,ClassList,ThreadIdList,PIDList: TStringList;
...
NumProcesses:=DataList.Count;
SetLength(ProcessInfo,NumProcesses);
for i:=0 to NumProcesses-1 do
begin
	DataList[i]:=UpperCase(DataList[i]);
...
Data:=GetFileNameFromhandle(hwnd);
DataList.Add(Data);
...
begin
	...
	repeat
		...
		begin
			if Pos(Filename,DataList[i])>0 then
			begin
				FileFound:=true;
				break;
			end;
...
begin
	...
	for i:=0 to DataList.Count-1 do
	begin
		if Pos(Filename,DataList[i])>0 then break;
	end;
	if i<DataList.Count then
	begin
		if not KillProcessByPID(StrToInt64(PIDList[i])) then
