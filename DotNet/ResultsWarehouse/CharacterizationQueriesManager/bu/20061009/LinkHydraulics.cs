...
namespace SystemsAnalysis.Characterization
{
	...
	public class LinkHydraulics : DictionaryBase
	{
		...
		public LinkHydraulics(Links links, int scenarioID, string modelCatalogDB)
		{			
			...
			{
				if ( (int)linkHydraulicsReader["scenarioID"] == scenarioID )
				{
		...
		public int PipeSurchargeCount(double min, double max)
		{
			...
			foreach (LinkHydraulic lh in this.Values)
			{
				...
				if (maxSurcharge >= min && maxSurcharge <= max)											
				{
					count++;
				}
			}
