...
namespace SystemsAnalysis.Analysis.CostEstimator.Classes
{
    ...
    private SortedList<double, decimal> _DepthToTrenchExcavationCostPerCuYd =
      new SortedList<double, decimal>();
		...
		/// <returns>Pipe material</returns>
		static public PipeMaterial StringToPipeMaterial(string materialName)
		{
      ...
      while (xpathIter.MoveNext())
      {
		...
		public double OutsideDiameterFromInsideDiameter(PipeMaterial material, double insideDiameter, out bool outsideTable)
		{
			...
			else if (insideDiameter < _InsideDiameterToOutsideDiameterTable[material].Keys[0])
			{
				outsideTable = true;
				return _InsideDiameterToOutsideDiameterTable[material].Values[0];
			} // if
			else if (insideDiameter > _InsideDiameterToOutsideDiameterTable[material].Keys[_InsideDiameterToOutsideDiameterTable[material].Count-1])
			{
				outsideTable = true;
		...
		public double ManholeDiameterFromInsideDiameter(double insideDiameter, out bool outsideTable)
		{
			...
			else if (insideDiameter > _InsideDiameterToManholeDiameterTable.Keys[_InsideDiameterToManholeDiameterTable.Count-1])
			{
				outsideTable = true;
		...
		public double TrenchWidthFromOutsideDiameter(double outsideDiameter, out bool outsideTable)
		{
			...
			else if (outsideDiameter > _OutsideDiameterToTrenchWidthTable.Keys[_OutsideDiameterToTrenchWidthTable.Count - 1])
			{
				outsideTable = true;
    ...
    public decimal TrenchExcavationCostPerCuYdFromDepth(double depth)
    {
      ...
      else
      {
		...
		public decimal PipeCost(PipeMaterial material, double diameter, out bool outsideTable)
		{
			...
			else if (diameter < _PipeCostTable[material].Keys[0])
			{
				outsideTable = true;
				return _PipeCostTable[material].Values[0];
			} // if
			else if (diameter > _PipeCostTable[material].Keys[_PipeCostTable[material].Count-1])
			{
				outsideTable = true;
		...
		public decimal LateralCost(double insideDiameter, double depth, out bool outsideTable)
		{
			...
			else if ((insideDiameter > _LateralCostTable.Keys[_LateralCostTable.Count-1]))
			{
				outsideTable = true;
			...
			else if (depth > _LateralCostTable.Values[diameterIndex].Keys[_LateralCostTable.Values[diameterIndex].Count - 1])
			{
				outsideTable = true;
		...
		public decimal ManholeCost(double pipeDiameter, double depth, out bool outsideTable)
		{
			...
			else if (finalManholeSize > _ManholeCostsTable.Keys[_ManholeCostsTable.Count - 1])
			{
				outsideTable = true;
		...
		public decimal FlowDiversionCost(double insideDiameter, out bool outsideTable)
		{
			...
			else if (insideDiameter > _FlowDiversionCostsTable.Keys[_FlowDiversionCostsTable.Count - 1])
			{
				outsideTable = true;
		...
		public decimal CIPPPipeCost(double insideDiameter, out bool outsideTable)
		{
			...
			else if (insideDiameter > _CIPPCostTable.Keys[_CIPPCostTable.Count - 1])
			{
				outsideTable = true;
		...
		public decimal PipeburstPipeCost(double insideDiameter, out bool outsideTable)
		{
			...
			else if (insideDiameter > _PipeburstCostTable.Keys[_PipeburstCostTable.Count - 1])
			{
				outsideTable = true;
      	...
      	case PipeMaterial.Concrete:
				case PipeMaterial.PVCHDPE:
					...
					else
					{
						if (_Depth < 18)
							baseCostMultiplierForDepth = 1.0;
						else if (_Depth >= 18 && _Depth < 20)
							baseCostMultiplierForDepth = 1.1;
						else if (_Depth >= 20 && _Depth < 24)
							baseCostMultiplierForDepth = 1.2;
						else if (_Depth >= 24)
