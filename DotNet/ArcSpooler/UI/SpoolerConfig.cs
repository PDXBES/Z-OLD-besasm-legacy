...
namespace ArcSpooler.UI
{
	public class SpoolerConfig
	{
		...
		public MasterDataFrame MasterDataFrame { get; private set; } // MasterDataFrame
    ...
    /// <summary>
		...
		private void ReadMasterDataFrameConfig()
		{
			...
			XElement masterDataFrameNode = _TemplateNode.Descendants("masterDataFrame").First();
			MasterDataFrame = new MasterDataFrame()
			{
				Name = masterDataFrameNode.Attribute("name").Value,
				LayerToShift = masterDataFrameNode.Attribute("layerToShift").Value,
				LayerKeyField = masterDataFrameNode.Attribute("layerKeyField").Value,
				ZoomToObject = Convert.ToBoolean(masterDataFrameNode.Attribute("zoomToObject").Value),
				RotateMap = Convert.ToBoolean(masterDataFrameNode.Attribute("rotateMap").Value),
				ScaleInterval = Convert.ToDouble(masterDataFrameNode.Attribute("scaleInterval").Value)
			};

			IEnumerable<XElement> highlightLayerNodes = 
				masterDataFrameNode.Descendants("highlightLayer");
